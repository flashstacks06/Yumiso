...
volatile bool flag_newList = false;
volatile bool nullData = false;
//volatile bool saveConfig = false;
volatile bool fire_stream = false;
volatile bool service_stream = false;
volatile bool update_events = false;
String route = "/registros/" + obj["id"].as<String>();// + "/actual";
FirebaseData stream;
FirebaseData stream_list;
...
// ----------------------------------------------------------------------------------- servicioStreamCallback
void servicioStreamCallback(FirebaseStream data)
{
  // Add config from spiff file

  Serial.println("{\"new_list\":true}");
  ...
  if ((strcmp(data.dataPath().c_str(), "/") == 0) && (strcmp(data.eventType().c_str(), "patch") != 0))
  {
    if (strcmp(data.dataType().c_str(), "null") != 0)
    {
      ...
      saveConfig = true;
      flag_newList = true;
      deserializeJson(doc_list, data.payload().c_str());
...
void prepareData()
{
  ...
  json.clear();
  //json.set("updatedBySelf", true);
  route = "/registros/" + obj["id"].as<String>() ; //+ "/data/" + String(now.year()) + "_" + String(now.month());
...
void connectFirebase()
{
  ...
  if (!Firebase.ready()) // Add more filters
  {
    ...
    // ---------- ------------------ Revision de archivo de configuracion
    route = "/registros/" + obj["id"].as<String>();
  ...
  // ---------- ------------------ Revision de Lista de Servicios
  if ((!service_stream) /*|| (!fire_events)*/)
  {
    if (!Firebase.RTDB.beginStream(&stream_list, route + "/servicios"))
      Serial.printf("config sream begin error, %s\n\n", stream_list.errorReason().c_str());
    else
    {
      Firebase.RTDB.setStreamCallback(&stream_list, servicioStreamCallback, streamTimeoutCallback);
      service_stream = true;
    }
